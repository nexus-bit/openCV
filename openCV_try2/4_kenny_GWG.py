...
cap = cv2.VideoCapture('video.mp4')  # "파일명.mp4" 가능
...
while (True):
    ...
    if lines is not None:
        ...
        if ((GWGP_CNT != 0)and(GWGM_CNT != 0)):#선이 두개다 보일 떄 직진
        ...
        elif ((GWGP_CNT != 0)and(GWGM_CNT == 0)):#왼쪽 선만 보일떄 우회전
            DIR = -1 * GWGP_SUM / GWGP_CNT
            print(DIR)
        elif ((GWGP_CNT == 0)and(GWGM_CNT != 0)):#오른쪽 선만 보일떄 좌회전
