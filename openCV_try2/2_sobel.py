...
cap = cv2.VideoCapture('highway.jpg')  # "파일명.mp4" 가능
...
while (True):
    ...
    #엣시 검출 하는 곳###############################################
    #소벨 엣지 검출
    img_gray = cv2.cvtColor(filt, cv2.COLOR_BGR2GRAY)
    ...
    edge = cv2.addWeighted(img_sobel_x, 1, img_sobel_y, 1, 0);
    ...
    #흑백-col으로 변환-cdst
    gray = cv.cvtColor(blur, cv.COLOR_GRAY2BGR)
    cdst = np.copy(gray)
    ...
    cdstP = np.copy(cdst)
    # 허프 라인 검출##################################################
    lines = cv.HoughLines(blur, 1, np.pi / 180, 150, None, 0, 0)
    ...
    # 허프 P 라인 검출
    linesP = cv.HoughLinesP(blur, 1, np.pi / 180, 50, None, 50, 10)
