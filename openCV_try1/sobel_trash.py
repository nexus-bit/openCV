...
while (True):
    ...
    ## 흰색 라인 필터
    #lower_blue = (0, 10, 165)
    #upper_blue = (50, 60, 235)
    #white = cv2.inRange(img_hsv, lower_blue, upper_blue)
    # 두개를 뙇
    #colorfilter = cv.addWeighted(yello, 0.5, white, 0.5, 0)
    ...
    linesP = cv.HoughLines(dst, 1, np.pi / 180, 140, None, 0, 0)
    if linesP is not None:  # 라인이 있을 경우에만 실행
        ...
        mpt1 = [0, 0]
        mpt2 = [0, 0]
        for i in range(0, len(linesP)):  # 삼각함수 구하는 과정임
            rho = linesP[i][0][0]
            theta = linesP[i][0][1]
            a = math.cos(theta)
            b = math.sin(theta)
            ...
            if (GWG < -0.01):  # 기울기가 양수일 경우
                ...
                plusum = (int(y0 + 1000 * (a)) - int(y0 - 1000 * (a))) / (
                int(x0 + 1000 * (-b)) - int(x0 - 1000 * (-b)))  # 기울기
                ...
                minusum = (int(y0 + 1000 * (a)) - int(y0 - 1000 * (a))) / (
                int(x0 + 1000 * (-b)) - int(x0 - 1000 * (-b)))
